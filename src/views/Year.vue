<template>
    <div class="container">
        ...
        <div class="quiz">
            <span class="question">该藏品来自哪个朝代呢？</span>
            <RadioGroup v-model="select" class="selectGroup">
                <Radio  label=1 style="margin-right:50px">{{answer1}}</Radio>
                <Radio  label=2 >{{answer2}}</Radio>
            </RadioGroup>
...
export default {
    ...
    mounted(){
        ...
        this.answer1 = status_list[id].year1
        this.answer2 = status_list[id].year2
    },
    methods: {
        word: function(score){
            if(score==0){
        ...
        submit: function(){
            var _this = this
            var id = this.quizList[0]
            if(this.select == status_list[id].year_answer){
